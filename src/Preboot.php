...
class Preboot
{
    ...
    public const STEP_MONITOR = 'Monitor';
    ...
    public static function go(?string $instance, ?string $mode)
    {
        ...
        if (
            ...
            self::$steps[self::STEP_AUTOLOAD] = null; //#TODO delegate this step adding mechanism to Mode Class
        }
        ...
        foreach(self::$steps as $prebootOption => $value) {
            if (is_null($value)) { //#TODO, don't ignore if null, let the plugin decide based on value and instance
    ...
    protected static function step($step, $instance = 'INTERNAL', $mode = 'INIT')
    {
        ...
        if (key_exists($step, self::$steps) && self::$steps[$step]) {
            ...
            if (
                ...
                || (is_array(self::$steps[$step]) && in_array($ident, self::$steps[$step]))
            ) {
        ...
        switch ($step) {
            case self::STEP_LIBXML :
                if (function_exists('libxml_use_internal_errors')) {
                    ...
                    Agent::meditate('libxml_use_internal_errors() not supported.', 'NOTICE');
                }
                break;
            case self::STEP_TZ :
