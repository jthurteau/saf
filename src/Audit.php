...
class Audit
{
    ...
    protected static function auditFail(?string $additional = ''): void
    {
        (trim($additional) != '') && ($additional = " {$additional}");
        $count = Cache::get('auditFailCount', null);
        if (is_null($count) || is_bool($count)) {
			Debug::out('starting fresh auditFailCount...');
            $count = 0;
        }
        Cache::store('auditFailCount', ++$count) || Debug::out('failed to store failure. simple cache callback not set');
    ...
    public static function update($id, $request = null, $user = null)
    {
        try {
            ...
            if (!$result) {
                self::auditFail('update query fail.');
            }
            return $result;
        } catch (\Error | \Exception $e){
            if (self::$critical) {
